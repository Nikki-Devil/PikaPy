...
def move(map_name_, tr, dg, si, coords, lang_alone, nb_save):
    ...
    def up(map_g, map_lines, map_values):
        ...
        elif map_lines[p_line[0] - 1][p_line[1]] == "a" or map_lines[p_line[0] - 1][p_line[1]] == " ":
            map_lines[p_line[0]] = map_lines[p_line[0]].replace("P", "a")
            map_lines[p_line[0] - 1] = map_lines[p_line[0] - 1][:(p_line[1])] + "P" + map_lines[
                                       p_line[0] - 1][p_line[1] + 1:]
            map_ = map_.replace("P", " ")
            map_dummy = map_lines[p_line[0] - 1].replace("a", " ")
